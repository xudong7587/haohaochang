...
export async function saveSongMetadata(
  ...
  input,
  cache,
  { required = true, idle = true } = {},
) {
  return withSongWrite(
    store,
    id,
    async (song) => {
      const key = publicationKey(id, "metadata");
      if (key && store.get(key)) return song;
      if (store.db.prepare("SELECT id FROM queue WHERE song_id=?").get(id))
        throw Object.assign(new Error("请先将歌曲移出播放队列"), {
          status: 409,
        });
      const patch = {};
      for (const key of ["title", "artist"])
        if (input[key] !== undefined) {
          patch[key] = String(input[key]).trim().slice(0, 120);
          if (!patch[key]) throw new Error("请填写歌名和歌手");
        }
      if (input.lyrics !== undefined)
        patch.lyrics = String(input.lyrics).slice(0, 25000);
      for (const key of [
        "tags",
        "tags_manual",
        "mode",
      ...
      if (patch.title || patch.artist)
        patch.search = searchText(
      ...
      try {
        if (input.lyricsSource)
          stage.store.set("lyrics-match:" + id, input.lyricsSource);
