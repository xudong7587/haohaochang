...
export async function download(job,payload,context){
  const {db,get,set,store,dir,roots,downloads,cache,legacyCache,emit,addJob,enqueue,fail}=context;
      if (job.kind === 'download') {
        ...
        addJob('import',{file,signature:info.size+':'+info.mtimeMs,title:payload.title,candidate:payload.candidate,sourceUrl:payload.url,enqueue:payload.enqueue,name:payload.name,isBacking:payload.isBacking});
      }
}
