...
export async function acquire(job,payload,context){
  const {db,get,set,store,dir,roots,downloads,cache,legacyCache,emit,addJob,enqueue,fail}=context;
      if(job.kind==='acquire'||job.kind==='find-video'){
        ...
        if(result.review){db.prepare("UPDATE jobs SET status='review',payload=?,error=? WHERE id=?").run(JSON.stringify({...payload,metadata:result.metadata,candidatePath:result.candidatePath,candidate:result.candidate}),result.review,job.id);emit('library',{});emit();return 'review';}
        if(job.kind==='find-video'&&result.file){const song=db.prepare('SELECT * FROM songs WHERE id=?').get(payload.id);await replaceVideo(store,song,result.file,result.sourceUrl||'',cache,{confirmed:true,offset:result.offset});}
