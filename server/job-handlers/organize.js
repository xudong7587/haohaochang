  ...
  } = context;
  {
    ...
    let meta = payload.approved
      ...
      : {
          ...
          artist: song.artist,
          lyrics: song.lyrics,
          needs_review: song.needs_review,
        };
    if (
      !payload.approved &&
      (meta.needs_review || meta.artist === "未知歌手")
    ) {
      meta = { ...meta, ...identifyTitle(song.artist + " - " + song.title) };
      if (meta.needs_review && get("enrichment", {}).enabled)
        meta = { ...meta, ...(await enrichSong(get("enrichment"), song)) };
    }
    if (!meta.needs_review && !meta.lyrics) {
      try {
        const result = await findLyrics(meta.title, meta.artist, song.duration);
        meta.lyrics = result.lyrics;
        meta.lyricsSource = {
          ...result,
          lyrics: undefined,
          status: "candidate",
    ...
    song = db.prepare("SELECT * FROM songs WHERE id=?").get(song.id);
    if (
      song.mode === "original" ||
      (song.mode === "separated" &&
