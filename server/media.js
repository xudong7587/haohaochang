...
import {resourceRoot,resourceFolder,preserveSource,archiveVersion} from './assets.js';
  ...
  async function walk(dir, root) {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) await walk(file, root);
        ...
        await withSongWrite(store,id,async song=>{
          if(store.db.prepare('SELECT id FROM queue WHERE song_id=?').get(id))return;
          const patch={};
          if(!['手动','AI'].includes(song.metadata_source))Object.assign(patch,{title,artist,metadata_source,needs_review});
          if(!song.tags_manual&&song.metadata_source!=='AI')patch.tags=JSON.stringify(tags);
          if(!song.lyrics){try{const lrc=await safeMedia(path.join(dir,path.parse(entry.name).name+'.lrc'),[root]);if((await stat(lrc)).size<100000)patch.lyrics=await readFile(lrc,'utf8');}catch{}}
          if(Object.entries(patch).some(([key,value])=>song[key]!==value))await saveSongMetadata(store,id,patch,resourceRoot(root),{required:false,idle:false});
          store.db.prepare('UPDATE songs SET poster=? WHERE id=?').run(poster,id);
          if(/\.(mp3|flac|wav|m4a|ogg|aac)$/i.test(entry.name)&&!store.get('video-source:'+id)?.keepAudio)store.db.prepare('UPDATE songs SET needs_video=1 WHERE id=?').run(id);
        },{wait:true});
      }
    }
  }
...
export async function prepareSong(store, id, roots, cache) {
  return withSongWrite(store,id,async()=>{
