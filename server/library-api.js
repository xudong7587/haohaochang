...
import {resourceManifest,canEnqueue,resourceNames} from './resource-manifest.js';
...
export function libraryApi({app,admin,member,store,cache,downloads,addJob,emit}){
 ...
 app.get('/api/admin/library',admin,(req,res)=>res.json(db.prepare('SELECT * FROM songs ORDER BY created DESC').all().filter(s=>!!get('hidden:'+s.id)===(req.query.hidden==='true')).map(s=>{const manifest=resourceManifest(store,s,cache);return {...s,tier:manifest.tier,manifest,missing:manifest.missing,sourceUrl:get('video-source:'+s.id)?.url||get('source:'+s.id)?.url||'',folder:get('package:'+s.id,'尚未生成新格式')};})));
 app.get('/api/catalog',member,(req,res)=>{
  ...
  res.json(unique.filter(s=>searchText(s.title,s.artist).includes(q)).slice(0,300).map(s=>({...s,localId:db.prepare('SELECT * FROM songs WHERE title=? AND artist=?').all(s.title,s.artist).find(song=>canEnqueue(store,song,cache))?.id||null})));
 });
 ...
 app.post('/api/admin/source-info',admin,async(req,res)=>{
  let candidate=await previewSourceCandidate(req.body.url,get('favorites',{}).cookie);
  if(candidate.identity.needs_review&&get('enrichment',{}).enabled){const parsed=await enrichSong(get('enrichment'),{title:candidate.externalTitle,sourceUrl:candidate.canonicalUrl});candidate=createSourceCandidate({...candidate,reviewReasons:[],evidence:[...candidate.evidence,...(parsed.evidence||[])]},parsed);}
  const candidateId=randomUUID();previews.set(candidateId,{candidate,created:Date.now()});
  if(previews.size>200)previews.delete(previews.keys().next().value);
  res.json({...candidate.identity,videoTitle:candidate.externalTitle,duration:candidate.duration,url:candidate.canonicalUrl,candidate,candidateId});
 });
 ...
 app.post('/api/admin/inbox-link',admin,async(req,res)=>{
  const url=canonicalVideo(req.body.url);let candidate;
  if(req.body.candidateId){const preview=previews.get(req.body.candidateId);if(!preview||Date.now()-preview.created>3600000)throw new Error('链接预览已过期，请重新解析');candidate=preview.candidate;if(candidate.canonicalUrl!==url)throw new Error('链接与已预览候选不一致，请重新解析');}
  else candidate=await previewSourceCandidate(url,get('favorites',{}).cookie);
  res.json({id:addJob('download',{url,title:candidate.externalTitle,candidate,enqueue:false})});
 });
 ...
 app.get('/api/playback-assets/:id',member,async(req,res)=>{
  ...
  res.json(resourceManifest(store,song,cache));
 });
 app.get('/api/assets/:id/:kind',member,async(req,res)=>{
  ...
  const revision=req.query.r===undefined?song.resourceRevision:Number(req.query.r);
  const dir=revision===song.resourceRevision?get('package:'+song.id):get('package-version:'+song.id+':'+revision);
  if(!dir)throw Object.assign(new Error('资源版本不存在'),{status:404});
  res.sendFile(await safeMedia(path.join(dir,names[req.params.kind]),[cache]));
 });
 app.post('/api/admin/library/:id/save',admin,async(req,res)=>{
  ...
  const updated=await saveSongMetadata(store,song.id,{title,artist,lyrics,lyricsSource:req.body.lyricsSource||{source:'手动 LRC',offsetUnit:'milliseconds'},expectedRevision:req.body.expectedRevision,metadata_source:'手动',needs_review:0},cache);
  if(req.body.prepare)addJob('prepare',{id:song.id});
  emit('library',{});res.json({ok:true,metadataRevision:updated.metadataRevision});
 });
 app.post('/api/admin/library/:id/source',admin,(req,res)=>{
  ...
  res.json({id:addJob('attach-video',{id:song.id,url,confirmed:req.body.confirmed===true,offset:Number(req.body.offset)||0})});
 });
 app.post('/api/admin/library/:id/restore',admin,(req,res)=>{currentSong(store,req.params.id);set('hidden:'+req.params.id,false);emit('library',{});res.json({ok:true});});
 app.delete('/api/admin/library/:id',admin,(req,res)=>{
  ...
  assertIdle(req.params.id);set('hidden:'+req.params.id,true);emit('library',{});res.json({ok:true,note:'已从曲库隐藏，原始媒体保留，可恢复'});
 });
}
