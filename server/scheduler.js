...
export function createScheduler(dependencies,{enabled=true,execute=runJob,onIdle=()=>{}}={}) {
  ...
  function addJob(kind, payload) {
    ...
    if(songId)payload={...payload,expectedRevision:payload.expectedRevision??currentSong(store,songId).metadataRevision};
    const id = randomUUID();
    payload={...payload,jobId:id,stagingId:id};
  ...
  async function work() {
    ...
    try {
      const context={...dependencies,addJob,enqueue};
      const songId=songIdFor(payload);
      const outcome=songId?await withSongWrite(store,songId,()=>execute(job,payload,context),{expectedRevision:payload.expectedRevision,jobId:job.id,wait:true}):await execute(job,payload,context);
      ...
      if(job.kind==='prepare'&&!resourceManifest(store,currentSong(store,payload.id),cache).playable) db.prepare("UPDATE songs SET status='error',error=? WHERE id=?").run(e.message.slice(-1800),payload.id);
    } finally { db.prepare('UPDATE jobs SET finished=? WHERE id=?').run(Date.now(),job.id);running--; emit('library', {});emit(); if(stopped&&!running)onIdle();else setImmediate(work); }
  }
  return {addJob,work,stop(){stopped=true;if(!running)onIdle();}};
}
