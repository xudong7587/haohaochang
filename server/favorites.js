export function favoriteConfig(input,old={}){
 ...
 const fields=['sessdata','bili_jct','buvid3','dedeuserid','ac_time_value'];const credentials=Object.fromEntries(fields.map(k=>[k,input.clearCookie?'':String(input[k]||old.credentials?.[k]||'').trim()]));
 ...
 if(Object.values(credentials).some(v=>/[;\r\n\t]/.test(v)))throw new Error('凭证字段不能包含分号或换行');
 const assembled=input.cookie?cookie:supplied?`SESSDATA=${credentials.sessdata}; bili_jct=${credentials.bili_jct}; buvid3=${credentials.buvid3}; DedeUserID=${credentials.dedeuserid}`:cookie;
 return {enabled:input.enabled===true,favoriteId:id,cookie:assembled,credentials,intervalMinutes:Math.max(5,Math.min(1440,Number(input.intervalMinutes)||10))};
}
