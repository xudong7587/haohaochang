...
export async function acquireSong(payload,{store,downloads,roots,outputs,search=onlineSearch}){
  const preview=payload.candidate?metadataFromCandidate(payload.candidate):{};
  const title=String(payload.metadata?.title||preview.title||payload.title||'').trim();
  const artist=String(payload.metadata?.artist||preview.artist||payload.artist||'').trim();
  ...
  const found=payload.candidate?[createSourceCandidate(payload.candidate)]:payload.sourceUrl?[createSourceCandidate({url:payload.sourceUrl,title:`${artist} ${title}`,identity:{title,artist,needs_review:1}})]:(await candidates(title,artist,false,search)).map(item=>createSourceCandidate(item));
  if(!found.length)return {review:'未找到可靠匹配。请补充资源链接，或检查歌手、歌名后重试。',metadata:{title,artist}};
  let last;
  for(const candidate of found.slice(0,3)){
    if(candidate.reviewReasons.length&&!payload.approved&&!payload.confirmed)return {review:'来源的歌手或录音版本需要核对，请确认此候选后继续。',metadata:{title,artist,lyrics:payload.metadata?.lyrics||''},candidate};
    try{
      await mkdir(downloads,{recursive:true});const id=createHash('sha256').update(candidate.canonicalUrl).digest('hex').slice(0,24);
      const file=path.join(downloads,id+'.m4a');
      try{await stat(file);}catch{
        await withBiliCookie(store.get('favorites',{}).cookie,path.join(downloads,'.credentials'),cookieFile=>run(process.env.YTDLP||'yt-dlp',[...(process.env.YTDLP_FFMPEG?['--ffmpeg-location',process.env.YTDLP_FFMPEG]:[]),...(cookieFile?['--cookies',cookieFile]:[]),'--ignore-config','--no-playlist','--socket-timeout','20','--retries','2','--max-filesize','200M','-x','--audio-format','m4a','-o',path.join(downloads,id+'.%(ext)s'),'--',candidate.canonicalUrl],900000));
      }
      const mode='original';
...
export async function findVideo(payload,{store,downloads,outputs,search=onlineSearch,download=downloadVideo,inspect=probe}){
  const song=store.db.prepare('SELECT * FROM songs WHERE id=?').get(payload.id);
  if(!song||!song.needs_video)return {};
  const key='mtv-candidate:'+song.id,previous=store.get(key);
  const action=payload.action||(payload.confirmed||payload.approved?'confirm':'preview');
  if(action==='reject'){
    if(previous?.candidate)store.set('mtv-rejected:'+song.id,[...new Set([...store.get('mtv-rejected:'+song.id,[]),previous.candidate.canonicalUrl])]);
    store.set(key,null);return {rejected:true};
  }
  if(!['preview','confirm','research'].includes(action))throw new Error('未知候选操作');
  const metadata={title:song.title,artist:song.artist};
  const url=action==='research'?null:payload.candidate?.canonicalUrl||payload.sourceUrl||payload.url;
  // Explicit URLs and staged paths always win. Confirm never launches a search.
  const samePath=!payload.candidatePath||previous?.path&&path.resolve(previous.path)===path.resolve(payload.candidatePath);
  let selected=action==='research'?null:(payload.candidate||(samePath&&(!url||previous?.candidate?.canonicalUrl===canonicalVideo(url))?previous?.candidate:null));
  let candidatePath=action==='research'?null:payload.candidatePath||(!url||previous?.candidate?.canonicalUrl===canonicalVideo(url)?previous?.path:null);
  if(action==='confirm'&&!selected&&!candidatePath&&!url)throw new Error('请先选择候选视频，再确认版本与偏移');
  if(url&&!selected)selected=createSourceCandidate({url,title:`${song.artist} ${song.title}`,identity:{...metadata,needs_review:1}});
  if(candidatePath){
    const file=await safeMedia(candidatePath,[path.join(outputs,'mtv-candidates',song.id),downloads]);
    const info=await inspect(file);if(!info.hasVideo)throw new Error('候选文件不含视频画面');
    if(!selected)selected=createSourceCandidate({provider:'local',title:`${song.artist} ${song.title}`,duration:info.duration,identity:{...metadata,needs_review:1}});
    return candidateResult(file,createSourceCandidate({...selected,duration:info.duration}));
  }
  if(search===onlineSearch)search=(q,p)=>onlineSearch(q,p,store.get('favorites',{}).cookie);
  const rejected=store.get('mtv-rejected:'+song.id,[]);
  const found=selected?[selected]: (await candidates(song.title,song.artist,true,search)).map(item=>createSourceCandidate(item)).filter(item=>!rejected.includes(item.canonicalUrl));
  let last;
  for(const candidate of found.slice(0,selected?1:2)){
    try{
      const {file}=await withBiliCookie(store.get('favorites',{}).cookie,path.join(downloads,'.credentials'),cookieFile=>download(candidate.canonicalUrl,downloads,cookieFile));const info=await inspect(file);if(!info.hasVideo)throw new Error('候选文件不含视频画面');
      ...
      return candidateResult(target,createSourceCandidate({...candidate,duration:info.duration}));
    }catch(error){last=error;}
  }
  if(selected&&last)throw last;
  return {review:'暂未找到可用 MTV。音频版本保持可用，可以继续唱；请补充视频链接。',metadata};

  function candidateResult(file,candidate){
    const reasons=[...candidate.reviewReasons,'recording-alignment-needs-review'];
    if(song.duration&&Math.abs(candidate.duration-song.duration)>4)reasons.push('duration-mismatch');
    candidate={...candidate,reviewReasons:[...new Set(reasons)]};
    store.set(key,{path:file,url:candidate.canonicalUrl,duration:candidate.duration,candidate});
    if(action==='confirm'){
      const offset=Number(payload.offset??0);if(!Number.isFinite(offset)||Math.abs(offset)>600)throw new Error('视频偏移必须为 -600 到 600 秒');
      return {file,sourceUrl:candidate.canonicalUrl,candidate,confirmed:true,offset};
    }
    return {review:'已保存 MTV 候选。请试听并核对录音版本和起始偏移，再确认关联画面。',metadata,candidatePath:file,candidate};
  }
}
