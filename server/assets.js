...
async function copyOnce(source, target) {
  ...
  try {
    ...
    if ((await stat(source)).size !== (await stat(temporary)).size)
      throw new Error("资源复制校验失败");
    try {
...
export async function migrateSongAssets(id, legacy, permanent) {
  ...
  for (const variant of ["vocal", "backing"]) {
    ...
    if (await exists(file))
      await copyOnce(file, path.join(permanent, path.basename(file)));
  }
  const stems = path.join(legacy, "stems", id);
  if (await exists(stems))
    for (const file of await readdir(stems)) {
      if (!/\.(wav|m4a|flac|mp3)$/i.test(file)) continue;
      await copyOnce(
        path.join(stems, file),
        path.join(permanent, "stems", id, file),
      );
    }
}
...
export async function preserveSource(song, permanent) {
  ...
  const signature = createHash("sha256")
    .update(JSON.stringify([song.path, info.size, info.mtimeMs]))
    .digest("hex")
    .slice(0, 12);
  const target = path.join(
    ...
    "sources",
    song.id,
    signature + path.extname(song.path),
  );
  await copyOnce(song.path, target);
  await writeFile(
    path.join(permanent, "sources", song.id, "song.json"),
    JSON.stringify(
      {
        id: song.id,
        title: song.title,
...
export async function archiveVersion(file) {
  ...
  const target = path.join(
    ...
    `${Math.trunc(info.mtimeMs)}-${info.size}${path.extname(file)}`,
  );
