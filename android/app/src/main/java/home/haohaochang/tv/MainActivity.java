...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            @Override public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
                if (request.isForMainFrame()) new AlertDialog.Builder(MainActivity.this).setTitle("暂时连不上 NAS").setMessage("请确认电视与 NAS 在同一网络，且 Docker 服务已经启动。").setPositiveButton("重试", (d,w) -> connect()).setNegativeButton("修改地址", (d,w) -> settings()).show();
            }
        });
    ...
    private void settings() {
        ...
        input.setSingleLine(true); input.setText(server == null ? "" : server); input.setHint("http://192.168.1.100:3210");
        input.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_VARIATION_URI);
        AlertDialog dialog = new AlertDialog.Builder(this).setTitle("连接家庭 NAS").setMessage("输入 NAS 的局域网地址。连接后用管理密码登录一次。遥控器菜单键可重新设置地址。").setView(input).setPositiveButton("连接", null).setNegativeButton("取消", (d,w) -> { if(server == null || server.isEmpty()) finish(); }).create();
        dialog.setOnShowListener(d -> dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            String value = input.getText().toString().trim().replaceAll("/+$", "");
            Uri uri = Uri.parse(value);
            if (!("http".equals(uri.getScheme()) || "https".equals(uri.getScheme())) || uri.getHost() == null || uri.getUserInfo() != null || uri.getQuery() != null || uri.getFragment() != null || (uri.getPath() != null && !uri.getPath().isEmpty())) { input.setError("请输入完整的 http://NAS-IP:3210 地址"); return; }
