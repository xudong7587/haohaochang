...
/** Native demux, decode, audio clock and SurfaceView output, retained from 0.4.4. */
...
final class NativePlayback {
  ...
  NativePlayback(Activity activity, FrameLayout root, Listener listener) {
    this.activity = activity;
    this.root = root;
    this.listener = listener;
    picture = new FrameLayout(activity);
    ...
    root.addOnLayoutChangeListener((v, l, t, r, b, ol, ot, or, ob) -> layout());
    handler.post(tick);
  }
  ...
  void command(JSONObject value) throws JSONException {
    ...
    if (action.equals("load")) {
      ...
      session = id;
      resources = media;
      legacy = value.optBoolean("legacy");
      failed.clear();
      error = "";
      warning = "";
      droppedFrames = 0;
    ...
    if (action.equals("state")) {
      lastCommand = SystemClock.elapsedRealtime();
      lease = value.optBoolean("lease");
      paused = value.optBoolean("paused", true);
      String variant = value.optString("variant", "backing");
      if (!variant.equals(requested)) {
        requested = variant;
        if (legacy) build(position());
        else selectAudio();
      }
      updatePlay();
      report();
    } else if (action.equals("seek") && player != null) {
      player.seekTo(Math.max(0, value.optLong("positionMs", 0)));
      ended = false;
      report();
  ...
  private void build(long positionMs) {
    ...
    try {
      if (player == null) {
        ...
        player =
            new ExoPlayer.Builder(activity, renderers)
                .setLoadControl(
        ...
        player.addAnalyticsListener(
            new AnalyticsListener() {
              ...
              public void onVideoDecoderInitialized(
                  EventTime time, String name, long timestamp, long duration) {
                decoder = name;
                report();
              }
              ...
              @Override
              public void onDroppedVideoFrames(EventTime time, int count, long elapsed) {
                droppedFrames += count;
              }
              ...
              public void onLoadError(
                ...
                for (String kind : new String[] {"video", "vocal", "backing"}) {
                  ...
                  if (item != null && info.dataSpec.uri.toString().equals(item.optString("url")))
                    failedLoad = kind;
                }
              }

              @Override
              public void onLoadCompleted(EventTime time, LoadEventInfo info, MediaLoadData data) {
      ...
      sources.add(source(audio, C.TRACK_TYPE_AUDIO));
      sourceKinds.add(audio);
      if (!legacy) {
        if (resources.has("video") && !failed.contains("video")) {
          sources.add(source("video", C.TRACK_TYPE_VIDEO));
          sourceKinds.add("video");
        }
        for (String kind : new String[] {"vocal", "backing"}) {
          if (!kind.equals(audio) && resources.has(kind) && !failed.contains(kind)) {
            sources.add(source(kind, C.TRACK_TYPE_AUDIO));
            sourceKinds.add(kind);
          }
        }
      }
      player.setTrackSelectionParameters(
          player.getTrackSelectionParameters().buildUpon().clearOverrides().build());
      player.setMediaSource(
          legacy
              ? sources.get(0)
              : new MergingMediaSource(true, false, sources.toArray(new MediaSource[0])),
          positionMs);
      player.prepare();
      updatePlay();
      layout();
  ...
  private void selectAudio() {
    ...
    for (Tracks.Group group : player.getCurrentTracks().getGroups()) {
      if (group.getType() != C.TRACK_TYPE_AUDIO
          || !group.getMediaTrackGroup().id.startsWith(sourceIndex + ":")) continue;
      chosen = wanted;
      if (!group.isSelected())
        player.setTrackSelectionParameters(
            player
  ...
  private void recover(PlaybackException exception) {
    long time = position();
    boolean pictureFailure =
        exception instanceof ExoPlaybackException
            && ((ExoPlaybackException) exception).type == ExoPlaybackException.TYPE_RENDERER
            && ((ExoPlaybackException) exception).rendererFormat != null
            && androidx.media3.common.MimeTypes.isVideo(
                ((ExoPlaybackException) exception).rendererFormat.sampleMimeType);
    ...
    if (kind.equals("video") && !legacy) {
      ...
      warning = "当前音轨不可用，已切换到可用音轨";
      build(time);
    } else {
      error = "原生播放失败（" + exception.getErrorCodeName() + "），请重试";
      report();
    }
  }
  ...
  private void updatePlay() {
    if (player != null) {
      boolean allowed =
          PlaybackPolicy.mayPlay(
                  foreground, lease, paused, SystemClock.elapsedRealtime() - lastCommand)
              && !ended
              && error.isEmpty();
      ...
      new Runnable() {
        ...
        public void run() {
          ...
          if (player != null && !session.isEmpty()) {
            ...
            double duration =
                audio == null ? 0 : audio.optDouble("duration", 0) - audio.optDouble("offset", 0);
            ...
            if (SystemClock.elapsedRealtime() >= nextReport) {
              report();
              nextReport = SystemClock.elapsedRealtime() + 200;
            }
          }
  ...
  private void layout() {
    ...
    picture.setVisibility(
        foreground
                && player != null
                && (legacy || (resources.has("video") && !failed.contains("video")))
            ? View.VISIBLE
            : View.INVISIBLE);
  }
    ...
    try {
      JSONObject state =
          new JSONObject()
              ...
              .put("playing", player != null && player.isPlaying())
              .put("ended", ended)
              .put("variant", chosen)
              .put("decoder", decoder)
              ...
              .put("pictureError", failed.contains("video"))
              .put("warning", warning)
              .put("error", error);
      listener.onState(state);
    } catch (JSONException ignored) {
      /* Only finite numeric state is emitted. */
    }
  }

  void foreground(boolean value) {
    foreground = value;
    // Coming back requires a fresh lease command from the room coordinator.
