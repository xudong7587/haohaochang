...
try{
 ...
 await page.getByRole('button',{name:'全屏播放'}).click();await page.locator('.fullscreen-join img').waitFor();assert.equal(await page.evaluate(()=>!!document.fullscreenElement),true);await page.screenshot({path:path.join(dir,'web-room-fullscreen.png')});await page.getByRole('button',{name:'全屏播放'}).click();
 ...
 await page.locator('.stage-card').first().click();await page.waitForFunction(()=>document.querySelector('video').src.includes('/backing?'));assert.match(await page.locator('.now-playing').innerText(),/伴奏/);await page.getByRole('button',{name:'全屏播放'}).click();await page.getByRole('complementary',{name:'已点歌曲预告'}).waitFor();await page.screenshot({path:path.join(dir,'web-room-queue.png')});await page.locator('.fullscreen-queue').waitFor({state:'hidden',timeout:10000});assert.equal(await page.locator('.fullscreen-join img').count(),1);await page.getByRole('button',{name:'全屏播放'}).click();
