...
async function request(url,body){if(url==='/player/heartbeat'&&window.leaseFailure)throw Object.assign(new Error('测试：另一台设备持有播放会话'),{status:409});if(url==='/player/ended')window.endedCalls=(window.endedCalls||0)+1;return {ok:true};}
