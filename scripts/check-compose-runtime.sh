...
main=dict(environment=dict(KTV_DISCOVERY_ENABLED='0',KTV_TV_DISCOVERY_ENABLED='0',KTV_LOCAL_ONLY='1'),
    volumes=[dict(type='bind',source=str(Path(sys.argv[2])/'scripts'),target='/app/scripts',read_only=True)])
Path(sys.argv[1]).write_text(json.dumps(dict(services=dict(ktv=main))))
PY
