...
export function Spectrum({video}){
 ...
 useEffect(()=>{
  ...
  let raf;const bins=new Uint8Array(64),g=c.getContext('2d');if(!g)return;
  const draw=()=>{
   const track=v._playback?.selected();bins.fill(0);
   if(track&&!track.failed&&!track.blocked&&!track.el.paused&&v._audioContext?.state==='running')track.analyser?.getByteFrequencyData(bins);
