...
export function resourceUrl(url, { token = "", revision } = {}) {
  ...
  if (parsed.origin === origin && token)
    parsed.searchParams.set("token", token);
  if (revision !== undefined && revision !== null)
    parsed.searchParams.set("r", String(revision));
...
export function normalizeManifest(manifest, { songId, token = "" } = {}) {
  ...
  for (const kind of ["video", "vocal", "backing", "lyrics"]) {
    ...
    if (detail?.available === false || (!detail?.url && !manifest[kind]))
      continue;
    resources[kind] = {
      ...detail,
      url: resourceUrl(detail?.url || `/api/assets/${songId}/${kind}`, {
        token,
        revision: detail?.revision ?? manifest.revision,
      }),
  ...
  return (background?.images || [])
    ...
    .map((image) => ({
      ...
      url: resourceUrl(image.url, {
        token,
        revision: image.revision ?? background.revision,
      }),
    }));
}
