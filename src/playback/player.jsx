...
export function Player({current,playback,notify,reactions=[],join,queue=[],request,token,background}) {
  ...
  const [time,setTime]=useState(0),[showQueue,setShowQueue]=useState(false);
  ...
  function mediaStatus(status){if('blocked'in status)setBlocked(status.blocked);if('error'in status)setError(status.error);if('warning'in status)setWarning(status.warning);if(status.pictureError)setPictureError(true);}
  ...
  useEffect(()=>{setError('');setWarning('');setBlocked(false);setPictureError(false);setTime(0);},[current?.id]);
  ...
  useEffect(()=>{
    ...
    v.muted=false;v.src=`/api/media/${current.song_id}/${variant}?token=${encodeURIComponent(token)}`;v.load();
    const ready=()=>{if(!alive)return;v.currentTime=Math.min(position.current,v.duration||0);if(lease&&!playback.paused)v.play().then(()=>{if(alive)setBlocked(false);}).catch(()=>{if(alive)setBlocked(true);});};
    v.addEventListener('loadedmetadata',ready);
    return()=>{alive=false;v.removeEventListener('loadedmetadata',ready);v.pause();};
  ...
  return <section ref={container} className={`tv-player ${full?'is-full':''}`}>
    <div className="video-stage">
      <video ref={video} playsInline onEnded={()=>{if(manifest?.version!==2)ended(current?.id);}} onError={()=>{if(manifest?.version!==2&&current)setError('播放失败，请重试或检查 NAS 连接');}}/>
