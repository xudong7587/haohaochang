...
请先阅读 [四端使用说明](docs/USER-GUIDE.md)：包括 Docker 管理端、TV APK、手机扫码点歌互动、PC 人声分离，以及更新、反代和排错。可从 [测试版下载页](https://github.com/xudong7587/haohaochang/releases/tag/test-0.1.0) 获取安装文件和说明。
...
没有电视时，在后台右上角点击“打开网页歌房”，或者打开 NAS 的 `/play`，即可在电脑浏览器播放。页面独立于管理后台，提供与 TV 相同的随机原唱、点歌和互动。主人首次连接后，来宾扫描页面二维码即可加入，无需手输网址和密码；二维码使用后台配置的内网或反代地址。
...
在线搜索、媒体下载都依赖 NAS 的出网能力。YouTube 可能受网络限制；Bilibili 可能返回风控错误。本版可填写 B 站 Cookie，但不提供交互式账号登录、会员权益处理或验证码处理，也不保证任何链接都能下载。搜索失败时可以尝试粘贴公开视频链接，或将合法持有的文件放入本地曲库。
...
`data/settings.json` 保存后台设置和 AI 密钥；`data/ktv.sqlite` 保存曲库、客厅凭证、队列和任务；`/download` 保存新下载原文件（旧版 `data/downloads` 继续兼容）；`data/cache` 保存电视播放版本及分离中间文件；可选 AI 服务的数据位于 `data/separator`。下载源文件保留，正式曲库需要可写挂载以接收自动整理结果。
