...
async function fixture(t, { http = false } = {}) {
  ...
  const service = http
    ? createApp({
        ...
        roots: [root],
        downloads,
        adminToken: "contracts-test-password",
        worker: false,
      })
    : null;
  ...
  const f = {
    ...
    async request(route, body, method = "POST") {
      const response = await fetch(base + "/api" + route, {
        method,
        headers: {
          Authorization: "Bearer contracts-test-password",
    ...
    async packageHash(id = "song") {
      ...
      return Object.fromEntries(
        await Promise.all(
          ["原唱.m4a", "伴奏.m4a", "歌词.lrc"].map(async (name) => {
            try {
              return [
                name,
                createHash("sha256")
...
function schedulerFor(t, f, execute) {
  ...
  const scheduler = createScheduler(
    ...
    { execute, onIdle: finished.resolve },
  );
  f.beforeClose(async () => {
    scheduler.stop();
...
test(
  "changed separated source and failed accompaniment publication preserve the old matched pair",
  { timeout: 10000 },
  async (t) => {
    ...
    const before = f.song(),
      directory = f.store.get("package:song"),
      hashes = await f.packageHash();
    const short = await f.audio(path.join(f.downloads, "short.wav"), 0.1);
    await assert.rejects(
      publishBacking(f.store, f.song(), f.cache, short),
      /时长|资源校验/,
    );
    ...
    await assert.rejects(
      prepareSong(f.store, "song", [f.root], f.cache),
      /音频已改变/,
    );
...
test(
  "changed nonempty media is unavailable until prepare repairs and verifies it",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t);
    await f.ready();
    ...
    await writeFile(
      path.join(old, "原唱.m4a"),
      "corrupt media is still a nonempty file",
    );
...
test(
  "concurrent imports of identical content converge on one song and its correct package pointer",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t);
    ...
    const ids = await Promise.all([
      importMedia(f.store, first, f.downloads, f.root, {
        title: "FIRST",
        artist: "Artist",
        tags: [],
      }),
      importMedia(f.store, second, f.downloads, f.root, {
        title: "SECOND",
        artist: "Artist",
        tags: [],
      }),
    ...
    assert.equal(
      f.store.db.prepare("SELECT count(*) AS n FROM songs").get().n,
      1,
    );
...
test(
  "scan waits for an existing song write and saves discovered lyrics through the package service",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t);
    ...
    await writeFile(
      path.join(f.root, "周杰伦 - 晴天.lrc"),
      "[00:00.00]扫描发现的歌词",
    );
    const entered = deferred(),
      release = deferred();
    const writing = withSongWrite(f.store, "scan", async () => {
      entered.resolve();
      await release.promise;
    });
    await entered.promise;
    let scanned = false;
    const scanning = scanLibrary(f.store, [f.root]).then(() => {
      scanned = true;
    });
    try {
      await pause(100);
    ...
    assert.equal(
      await readFile(
        path.join(f.store.get("package:scan"), "歌词.lrc"),
        "utf8",
      ),
      f.song("scan").lyrics,
    );
...
test("metadata exports use the committed revision for no-op and provenance-only edits", async (t) => {
  ...
  const noop = await saveSongMetadata(
    f.store,
    "song",
    { title: f.song().title, expectedRevision: before },
    f.cache,
  );
  assert.equal(noop.metadataRevision, before);
  const exported = () =>
    readFile(
      path.join(f.store.get("package:song"), "歌曲信息.json"),
      "utf8",
    ).then(JSON.parse);
  assert.equal((await exported()).metadataRevision, noop.metadataRevision);
  const changed = await saveSongMetadata(
    f.store,
    "song",
    {
      metadata_source: "手动",
      evidence: '["confirmed"]',
      expectedRevision: noop.metadataRevision,
    },
    f.cache,
  );
...
test(
  "scheduler runs different songs concurrently and serializes different tasks for one song",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t);
    f.seed("a");
    ...
    const scheduler = schedulerFor(t, f, async (job, payload) => {
      assert.equal(
        running.has(payload.id),
        false,
        "same song must never overlap",
      );
    ...
    const ids = [
      scheduler.addJob("first-a", { id: "a" }),
      scheduler.addJob("second-a", { id: "a" }),
      scheduler.addJob("first-b", { id: "b" }),
    ];
    try {
    ...
    await until(
      () =>
        ids.every(
          (id) =>
            f.store.db.prepare("SELECT status FROM jobs WHERE id=?").get(id)
              .status === "done",
        ),
      "all scheduled jobs to finish",
    );
...
test(
  "failed job retry accepts its own committed revision but rejects a later user edit",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t);
    f.seed();
    ...
    const scheduler = schedulerFor(t, f, async (job, payload) => {
      ...
      if (calls === 1 || calls === 3) {
        await saveSongMetadata(
          f.store,
          payload.id,
          { title: "Job edit " + calls },
          f.cache,
          { required: false, idle: false },
        );
    ...
    assert.equal(
      JSON.parse(state(first).payload).expectedRevision,
      f.song().metadataRevision,
    );
    await retry(first);
    ...
    await saveSongMetadata(
      f.store,
      "song",
      { title: "Later user edit", expectedRevision: checkpoint },
      f.cache,
    );
    await retry(second);
    ...
    assert.equal(
      JSON.parse(state(second).payload).expectedRevision,
      checkpoint,
    );
...
test(
  "reopening a running job resumes committed phases without publishing another resource revision",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t);
    ...
    f.store.db
      ...
      .run(
        jobId,
        "prepare",
        JSON.stringify({ id: "song", expectedRevision: 0 }),
        "running",
        Date.now(),
      );
    const phases = () =>
      withSongWrite(
        ...
        {
          jobId,
          expectedRevision: JSON.parse(
            f.store.db.prepare("SELECT payload FROM jobs WHERE id=?").get(jobId)
              .payload,
          ).expectedRevision,
        },
      );
    ...
    assert.equal(
      f.store.db.prepare("SELECT status FROM jobs WHERE id=?").get(jobId)
        .status,
      "running",
    );
    f.reopen();
...
test("stale metadata and a staged publication conflict preserve the current resource pointer", async (t) => {
  ...
  f.seed();
  const before = f.song();
  const newer = await saveSongMetadata(
    f.store,
    "song",
    { title: "Newer title", expectedRevision: before.metadataRevision },
    f.cache,
  );
  ...
  await assert.rejects(
    saveSongMetadata(
      f.store,
      "song",
      { title: "Old draft", expectedRevision: before.metadataRevision },
      f.cache,
    ),
  ...
  try {
    f.store.db
      .prepare("UPDATE songs SET artist='External writer' WHERE id='song'")
      .run();
    assert.throws(
...
test(
  "legacy and new lyric APIs update the same files and roll back failed publication",
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t, { http: true });
    await f.ready();
    const legacy = await f.request(
      "/admin/songs/song/lyrics",
      {
        lyrics: "[00:00.00]旧入口新歌词",
        expectedRevision: f.song().metadataRevision,
      },
      "PUT",
    );
    ...
    assert.equal(
      await readFile(
        path.join(f.store.get("package:song"), "歌词.lrc"),
        "utf8",
      ),
      f.song().lyrics,
    );
    const updated = await f.request("/admin/library/song/save", {
      title: f.song().title,
      artist: f.song().artist,
      lyrics: "[00:00.00]新入口歌词",
      expectedRevision: f.song().metadataRevision,
    });
    ...
    const oldAsset = await fetch(
      `${f.base}/api/assets/song/lyrics?r=${oldRevision}`,
      { headers: { Authorization: "Bearer contracts-test-password" } },
    );
    ...
    const before = f.song(),
      ...
      hashes = await f.packageHash();
    f.store.db.exec(
      "CREATE TEMP TRIGGER reject_test_lyrics BEFORE UPDATE OF lyrics ON songs WHEN NEW.lyrics LIKE '%reject-test%' BEGIN SELECT RAISE(ABORT,'Simulated publication failure'); END;",
    );
    const failed = await f.request(
      "/admin/songs/song/lyrics",
      {
        lyrics: "[00:00.00]reject-test",
    ...
    const revisions = await readdir(
      path.join(f.store.get("package-base:song"), "资源版本"),
    );
    assert.equal(
      revisions.length,
      3,
      "failed publication must remove its staged directory",
    );
  },
);
