...
test("favorite pagination, credentials and canonical video links", async () => {
  const config = favoriteConfig({
    enabled: true,
    favoriteId: "https://space.bilibili.com/1/favlist?fid=123",
    cookie: "SESSDATA=test",
    intervalMinutes: 1,
  });
  assert.equal(config.favoriteId, "123");
  assert.equal(config.intervalMinutes, 5);
  const result = await favoritePage(config, 2, async (url, options) => {
...
test("OpenAI metadata response, provenance and uncertainty review", async (t) => {
  ...
  app.post("/responses", (req, res) => {
    assert.equal(req.get("authorization"), "Bearer test-only");
  ...
  const result = await enrichSong(config, { title: "raw input" });
  assert.deepEqual(result.tags, ["港台", "女声"]);
  assert.equal(result.needs_review, 0);
  assert.equal(result.evidence.length, 1);
  confidence = 0.4;
  assert.equal(
    (await enrichSong(config, { title: "unknown" })).needs_review,
    1,
  );
});
