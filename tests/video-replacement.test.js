...
test('replacing an MV publishes one active version, preserves usable audio, and rolls back failed candidates',async t=>{
 ...
 await assert.rejects(replaceVideo(store,current(),candidate,'test:new',cache),{code:'VIDEO_REVIEW_REQUIRED'});
 assert.equal((await replaceVideo(store,current(),candidate,'test:new',cache,{confirmed:true,offset:0})).keepAudio,true);
 ...
 await replaceVideo(store,current(),third,'test:third',cache,{confirmed:true,offset:0});dir=store.get('package:'+id);assert.notDeepEqual(await readFile(path.join(dir,'画面.mp4')),picture);assert.deepEqual(await readFile(path.join(dir,'原唱.m4a')),original);
