...
async function fixture(t) {
  ...
  store.db.prepare('INSERT INTO songs(id,path,title,artist,created) VALUES (?,?,?,?,?)').run('song', source, '测试', '测试', Date.now());
  ...
  const config = {endpoint:`http://127.0.0.1:${server.address().port}`, model:'test'};
  t.after(async () => {server.close(); store.db.close(); await rm(dir, {recursive:true, force:true});});
  return {dir, cache, source, vocal, song, remote, config, get store() {return store;},
    reopen() {store.db.close(); store = openStore(path.join(dir, 'db')); return store;}};
}

test('NAS restart resumes saved remote job without another upload', async t => {
  const f = await fixture(t);
  let submits = 0, polls = 0;
  f.remote.post('/separate', (req, res) => {submits++; res.json({id:'resume_job', status:'queued'});});
  f.remote.get('/jobs/resume_job', (req, res) => {polls++; if (polls === 1) return res.sendStatus(503); res.json({status:'done', instrumental_url:'/artifact'});});
  await assert.rejects(runProviderJob(f.store, f.song, f.vocal, f.cache, f.config), /503/);
  const result = await runProviderJob(f.reopen(), f.song, f.vocal, f.cache, f.config);
...
test('uncertain POST retry retains idempotency key; explicit failed result starts fresh request', async t => {
  ...
  f.remote.post('/separate', (req, res) => {
    ...
    if (keys.length === 2) return res.json({id:'failed_job', status:'failed', error:'Service restarted; retry from NAS'});
    res.json({status:'done', instrumental_url:'/artifact'});
  });
  await assert.rejects(runProviderJob(f.store, f.song, f.vocal, f.cache, f.config), /503/);
  await assert.rejects(runProviderJob(f.reopen(), f.song, f.vocal, f.cache, f.config), /Service restarted/);
...
test('invalid output is rejected before old accompaniment changes and staging is cleaned', async t => {
  ...
  await run(ffmpeg, ['-y', '-v', 'error', '-f', 'lavfi', '-i', 'sine=duration=0.2', short]);
  f.remote.post('/separate', (req, res) => res.json({status:'done', instrumental_url:'/short'}));
  ...
  assert.equal(f.store.db.prepare('SELECT mode FROM songs WHERE id=?').get('song').mode, 'original');
  ...
  await run(ffmpeg, ['-y', '-v', 'error', '-i', f.source, '-map', '0:a', '-map', '0:a', '-c:a', 'copy', dual]);
...
test('busy PC chooses cloud, but a PC checkpoint resumes its own job while busy', async t => {
  const f = await fixture(t);
  let pcSubmits = 0, cloudSubmits = 0, polls = 0;
  f.remote.get('/pc/health', (req, res) => res.json({protocol:'ktv-separation-v1', busy:true, pending:1}));
  f.remote.post('/pc/separate', (req, res) => {pcSubmits++; res.json({status:'queued', id:'pc_job'});});
  f.remote.get('/pc/jobs/pc_job', (req, res) => {polls++; if (polls === 1) return res.sendStatus(503); res.json({status:'done', instrumental_url:'/artifact'});});
  f.remote.post('/separate', (req, res) => {cloudSubmits++; res.json({status:'done', instrumental_url:'/artifact'});});
  f.store.set('ai', {enabled:true, ...f.config, pcEndpoint:f.config.endpoint + '/pc', pcModel:'test'});
  ...
  await assert.rejects(runProviderJob(f.store, f.song, f.vocal, f.cache, pc), /503/);
  await separateSong(f.store, f.song, f.cache);
...
test('missing accompaniment regenerates despite a stale separated database flag', async t => {
  const f = await fixture(t);
  let uploads = 0;
  f.remote.post('/separate', (req, res) => {uploads++; res.json({status:'done', instrumental_url:'/artifact'});});
  f.store.set('ai', {enabled:true, ...f.config});
  await separateSong(f.store, {...f.song, mode:'separated'}, f.cache);
  assert.equal(uploads, 1);
  assert.ok((await readFile(path.join(f.store.get('package:song'), '伴奏.m4a'))).length > 1000);
});
