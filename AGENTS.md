...
- 新格式为一份画面加独立原唱、伴奏和歌词。补画面不应默默替换音频；失败保留此前可用资源。当前偏差见审查报告。
...
- npm test 含真实 FFmpeg 测试，npm run build 构建前端。scripts/ui-check.mjs 当前有已知失败 R8；scripts/adversarial-check.mjs 退出 0 仅代表观察完成，不代表缺陷已修复。
