...
export function createSourceCandidate(info,identity=info.identity){
  ...
  const canonicalUrl=local?null:canonicalVideo(info.canonicalUrl||info.url);
  const url=local?null:new URL(canonicalUrl);
  const externalTitle=String(info.externalTitle||info.title||'').slice(0,500);
  const parsed=identity||identifyVideo({...info,title:externalTitle});
  const version=String(parsed.version||recordingVersion(externalTitle)||'').slice(0,120);
  const reviewReasons=[...new Set([...(Array.isArray(info.reviewReasons)?info.reviewReasons:[]),...(parsed.reviewReasons||[]),...(!parsed.title||!parsed.artist||parsed.artist==='未知歌手'||parsed.needs_review?['identity-unconfirmed']:[]),...(version?['recording-version-needs-review']:[])])];
  const duration=Number(info.duration);
  return {
    provider:local?'local':url.hostname==='www.bilibili.com'?'bilibili':'youtube',canonicalUrl,
    page:url?.hostname==='www.bilibili.com'?Number(url.searchParams.get('p')||1):null,
    externalTitle,duration:Number.isFinite(duration)&&duration>0?duration:null,
    identity:{title:String(parsed.title||'').slice(0,120),artist:String(parsed.artist||'未知歌手').slice(0,120),version,needs_review:reviewReasons.length?1:0},
    evidence:Array.isArray(info.evidence)?info.evidence.slice(0,20):[{kind:'platform-metadata',url:canonicalUrl,title:externalTitle,videoTitle:String(info.videoTitle||''),uploader:String(info.uploader||'')}],
    reviewReasons
  };
}
...
export function metadataFromCandidate(input){
  ...
  return {...candidate.identity,evidence:candidate.evidence,metadata_source:`${candidate.provider} 候选`,sourceUrl:candidate.canonicalUrl,sourceCandidate:candidate};
}
